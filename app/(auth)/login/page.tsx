...
export default function LoginPage() {
  ...
  const handleSendOtp = () => {
    ...
    setTimeout(() => { setLoading(false); setOtpSent(true); }, 900);
  };

  const handleVerifyOtp = async () => {
    if (otp !== "1234" && otp.length !== 4) { setError("Incorrect OTP. Use '1234' for testing!"); return; }
    setError(""); setLoading(true);
    try {
      const civilians = await db.getCivilians();
      const existing = civilians.find(c => c.phone === phone);
      if (existing) {
        localStorage.setItem("roadguard_session", JSON.stringify({ role: "civilian", userId: existing.id, name: existing.full_name, district: existing.district }));
        router.push("/civilian/dashboard");
      } else { setShowOnboarding(true); }
    } catch { setError("Database error. Proceeding to onboarding."); setShowOnboarding(true); }
  ...
  const handleOnboardingSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const newId = `civ-${Math.random().toString(36).substr(2, 9)}`;
      const newCiv = await db.createCivilian({ id: newId, full_name: onboardingForm.fullName, phone, district: onboardingForm.district, city: onboardingForm.city, pincode: onboardingForm.pincode, points_total: 100, streak_days: 1 });
      await db.createNotification({ target_role: "civilian", target_id: newId, title: "Welcome to சாலையின் குரல் TN! 🌳", body: "Thank you for joining. You have been awarded +100 welcome points.", type: "point_gain" });
      localStorage.setItem("roadguard_session", JSON.stringify({ role: "civilian", userId: newCiv.id, name: newCiv.full_name, district: newCiv.district }));
      router.push("/civilian/dashboard");
    } catch { setError("Failed to complete onboarding."); }
    finally { setLoading(false); }
  };

  const handleAdminLogin = async (e: React.FormEvent) => {
    e.preventDefault(); setError("");
    if (email === "state@tn.gov.in" && password === "admin123") bypassAdmin("adm-111");
    else if (email === "coimbatore@tn.gov.in" && password === "admin123") bypassAdmin("adm-222");
    else setError("Invalid credentials. Use the Demo Preset Buttons below!");
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row relative overflow-hidden bg-white text-slate-800">
      ...
      <motion.div
        ...
        initial="hidden"
        animate="show"
        className="flex-1 flex items-center justify-center p-6 md:p-12 z-10 bg-slate-50"
      >
            ...
            {!showOnboarding ? (
                ...
                <div className="flex p-1 rounded-xl bg-slate-100 border border-slate-200 mb-8">
                  {(["civilian", "admin"] as const).map((tab) => (
                    <motion.button
                      ...
                      className={`flex-1 py-2.5 rounded-lg text-sm font-semibold relative z-10 transition-colors ${
                  ...
                  {activeTab === "civilian" ? (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-slate-500">Login with your mobile number. No password required.</p>
                      </div>
                        ...
                        {!otpSent ? (
                          <motion.div key="phone" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
                            ...
                            <GlowButton
                              ...
                              icon={loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Phone className="w-4 h-4" />}
                            >
                          ...
                          <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            <div>
                              <label className="text-[10px] font-mono uppercase tracking-wider text-slate-400 block mb-1">Verify OTP (கடவுச்சொல்)</label>
                              <div className="relative">
                                <Key className="w-4 h-4 absolute left-3 top-3.5 text-slate-400" />
                                <input
                                  type="text" value={otp} maxLength={4}
                                  onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))}
                                  placeholder="Enter 1234 for Demo"
                                  className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm font-mono tracking-widest text-center transition-all"
                                />
                              </div>
                              <span className="text-[10px] text-slate-400 mt-1 block">OTP sent to <strong>+91 {phone}</strong></span>
                            </div>
                            <div className="flex gap-3">
                              <GlowButton variant="ghost" size="md" onClick={() => { setOtpSent(false); setOtp(""); }} className="flex-1">Back</GlowButton>
                              <GlowButton variant="primary" size="md" disabled={loading} onClick={handleVerifyOtp} className="flex-[2]">
              ...
              >
                <form onSubmit={handleOnboardingSubmit} className="space-y-5">
                  ...
                  >
                    <div className="w-14 h-14 rounded-full bg-primary/10 border-2 border-primary/20 flex items-center justify-center text-2xl mx-auto mb-3">
                      🎁
                    </div>
                    <h3 className="text-xl font-display font-extrabold text-[#1A3A5C]">Create Profile (பதிவு செய்தல்)</h3>
                    <p className="text-xs text-slate-500 mt-1">Enter details to unlock your <strong className="text-primary">+100 PTS</strong> signup bonus.</p>
                  </motion.div>

                  <AnimatePresence>
                    {error && (
                      <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="p-3 rounded-lg bg-red-50 border border-red-200 text-red-600 text-xs font-semibold">
                        {error}
                      </motion.div>
                    )}
                  </AnimatePresence>
                  ...
                  <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-4">
                    {[
                      { icon: User, label: "Full Name (முழு பெயர்)", type: "text", required: true, value: onboardingForm.fullName, onChange: (e: any) => setOnboardingForm(p => ({ ...p, fullName: e.target.value })), placeholder: "Selvam Kumar" },
                      { icon: MapPin, label: "City / Town / Village", type: "text", required: true, value: onboardingForm.city, onChange: (e: any) => setOnboardingForm(p => ({ ...p, city: e.target.value })), placeholder: "e.g. Kattur" },
                      { icon: Lock, label: "Pincode (அஞ்சல் குறியீடு)", type: "text", maxLength: 6, required: true, value: onboardingForm.pincode, onChange: (e: any) => setOnboardingForm(p => ({ ...p, pincode: e.target.value.replace(/\D/g, "") })), placeholder: "e.g. 641009" },
                    ].map((field, i) => (
